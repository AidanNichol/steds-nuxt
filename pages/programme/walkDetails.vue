...
<div id="walkData">
  <div id="walkDetail">
    ...
    <p v-if="walk.coffee"><span>Morning Coffee: </span>{{walk.coffee}}</p>
    <p v-if="walk.evening"><span>Evening Meal: </span>{{walk.evening}}</p>
    <p class="screenonly" v-if="walk.details === 'Y'">
      See <a :href="getWalkdataURL(walk.img)" target="_blank">Route Map</a> for these walks.
    </p>
    <div id="instructions" class="screenonly" v-if="walk.scaledPrf === 'Y'">
      The profiles for these walks are all to the same scale.<br/> The distances shown, under the profiles, are the distances remaining to the finish.
    </div>
    <div id="instructions" class="screenonly" v-else>
      The profiles for these walk are <b>not</b> to the same scale. (The blue vertical lines correspond with the waypoints on the route card.)
    </div>
  </div>

  <div id="walkDetails">
    <div class="walk" v-for="route in walk.routes">
      <div class="routeHeading">
        <strong>Walk {{route.no}}</strong> &nbsp;&nbsp;&nbsp;&nbsp;<span>Leader: </span>{{route.leader}}
      </div>
      <div class="profile">
        <img border="0" :src="getWalkProfileURL(route.prfImg)" alt="" :isize="route.imgSize" />
      </div>
      <div class="routeDetails"><span>Distance: </span>{{Math.round(route.distance,1)}} Mi <span>Ascent: </span>{{route.ascent}} m <span>Descent: </span>{{route.descent}} m</div>
      <div class="screenonly routeDetails links">
        <a :href="getWalkURL('getWalkRouteGpx', walk.date, route.no)" download target="_blank" title="Download route as gpx">
          <img src="~/assets/image/gpx.png" style="height=100px"/>
        </a>
        <nuxt-link :to="showRoute(route.no)">
          <img src="~/assets/image/OS.png" style="height=100px" title="View route on OS map"/>
        </nuxt-link>
      </div>
...
export default {
  ...
  methods: {
    ...
    // },
    showRoute(no) {
      return `/programme/OSMap?walkId=${this.walk.date}&no=${no}`;
